            ...
            if (projetId) {
                $.get('router2.php?action=getCreneauxDisponibles&projet_id=' + projetId, function(data) {
                    ...
                    if (creneaux.length > 0) {
                        ...
                        options = '<option value="">-- Aucun créneau disponible --</option>';
                        $('#creneau_id').html(options).prop('disabled', true);
                        $('#submitBtn').prop('disabled', true);
                    }
                });
            } else {
                $('#creneau_id').html('<option value="">-- Choisissez d'abord un projet --</option>')
                              .prop('disabled', true);
                $('#submitBtn').prop('disabled', true);
            }
        });
